...
def substrings(string)
  ...
  (0...string.length).each do |word_start|
    (word_start...string.length).each do |word_end|
      result << string[word_start..word_end]
    end
  end
  result
end
...
def subwords(word, dictionary)
  ...
  word_salad = substrings(word)

  word_salad.each do |str|
...
block = Proc.new { |sum, num| sum + num }
p [1,2,3].my_inject(&block)
